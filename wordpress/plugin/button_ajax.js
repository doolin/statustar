...
(function() {
	...
	var Button = {
		disable : function() {
			...
			if (document.getElementById("tweeted")) {
				retweet.innerHTML = '<span class="left"></span>' + document
						.getElementById("tweeted").value + '<span class="right"></span>'
			} else {
		...
		createPopup : function(url, x, y, title) {
			popup = function() {
				...
				window.open(url, "",
						"menubar=0,resizable=1,status=1,toolbar=0,location=0,width="
								+ x + ",height=" + y + ",top=" + top + ",left="
								+ left)
			};
			retweet.target = "";
	...
	var Event = {
		add : function(object, action, onSuccess) {
	...
	var Ajax = {
		request : function(url, params, onSuccess) {
			xmlhttp = null;
	...
	var params = new Array("source", "service", "service_api", "alias",
			"hashtags", "space", "advertid");
	var urls = new Array("http://twitter.com", "http://tweetmeme.com");
	var timer = 0;
	var xmlhttp;
	var tweet = Button.selector("tweetmemebutton", document
	...
	var user = (Cookie.read("user") != null && Cookie.read("user_token") != null) ? true
			: false;
	...
	if (c.innerHTML != "?") {
		Button.beenTweeted();
		if (!settings) {
			var url = "http://tweetmeme.com/popup/option";
			url += "?url_id=" + tweet.id.substr(1);
			url += Button.buildParams();
			Button.createPopup(url, 500, 340, "Enable Logged in Retweeting")
		} else {
			if (settings["tweetoption"] == "no") {
			} else {
				if (user) {
					if (settings["popup"] != "no") {
						...
						var clickfunction = function(e) {
							...
							var parameters = "url_id=" + tweet.id.substr(1);
							parameters += Button.buildParams();
							Ajax.request("/ajax/tweet", parameters, function() {
								if (xmlhttp && xmlhttp.responseText) {
									var data = eval("(" + xmlhttp.responseText
											+ ")");
									tweet.className = "tweetmemebutton";
									if (data.response != "failure") {
										Cookie.saveData(tweet.id.substr(1));
										Button.disable()
									} else {
										if (data.data == "Already tweeted.") {
											Button.disable();
											Cookie.saveData(tweet.id.substr(1))
										}
									}
						...
						retweet.onclick = clickfunction
					}
				} else {
					var url = "http://tweetmeme.com/auth/login?autotweet=1&r=";
					url += escape("/popup/option" + "?url_id="
							+ tweet.id.substr(1));
					url += Button.buildParams();
